...
def _route_after_reflect_sub(state: SubAgentState) -> str:
    ...
    Loop guards mirror the main-graph ``route_after_reflect`` (F12): once the
    plan is exhausted OR the iteration budget is hit, a low-confidence
    reflection must NOT bounce back to execute. Execute then has no remaining
    steps and the pair loops reflect↔execute until LangGraph's recursion
    limit crashes the run (GraphRecursionError). Accept the partial result
    (END) instead. battery-04 q07 Run 5: a delegated sub-agent finished its
    plan but reflect heuristically reported LOW confidence (it cannot
    self-verify the partial goal); without these guards that burned a tight
    sub-second reflect↔execute loop.
    """
    ...
    # HARD CAP GUARD: never re-enter execute from reflect once the sub-agent's
    # iteration budget is exhausted — accept the partial result and END.
    iteration_count = state.get("iteration_count", 0)
    max_iterations = state.get("max_iterations", 10)
    if iteration_count >= max_iterations:
        logger.info(
            f"Sub-agent max iterations ({max_iterations}) reached on reflect; "
            f"accepting partial result (END)"
        )
        return "__end__"
    ...
    # Low confidence → retry execute, BUT only if steps remain. A finished
    # plan reported low-confidence must END (accept the partial result) —
    # retrying execute when there are no remaining steps is an infinite
    # reflect↔execute loop (execute returns immediately, reflect fires again).
    if confidence in {Confidence.VERY_LOW, Confidence.LOW}:
        ...
        if not has_remaining_steps:
            logger.info(
                "Sub-agent reflect low-confidence but plan exhausted "
                "(no remaining steps); accepting partial result (END) "
                "instead of retrying execute"
            )
            return "__end__"
        return "execute"
