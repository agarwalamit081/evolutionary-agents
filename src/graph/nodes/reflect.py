...
def _ground_should_evolve(
    ...
    in objective evidence (for deliverable goals, the artifact actually on disk)
    makes evolution fire on genuine successes while preserving a model's own
    ``should_evolve=True`` via OR.

    Recovered-error successes evolve too. battery-03 showed evolution firing
    ONLY on pristine clean runs (8 cycles/5 deployed) because a blanket
    ``if errors: return False`` gate sat ahead of the deliverable check — so the
    richest learning signal (a run that overcame transient errors yet still
    produced its deliverable) was discarded. The deliverable-on-disk check now
    runs FIRST: a deliverable goal fires whenever the artifact is on disk,
    regardless of transient errors. The errors-gate survives only for
    non-deliverable goals (no artifact confirms success → a logged error means
    the run is ambiguous/failed → never crystallize it).
    ...
    if goal_deliverable is not None:
        ...
        # when is_complete=True). Fire regardless of step count AND regardless
        # of transient errors — a run that recovered from errors yet still
        # produced its deliverable is exactly the success worth learning from.
        return _deliverable_on_disk(goal_deliverable)
    # Non-deliverable goal: no artifact confirms success, so a logged error
    # means the run is ambiguous/failed — never crystallize it. A clean run
    # still needs ≥3 completed steps AND HIGH+ confidence so a trivial/empty
    # success does not evolve. (Reached at route_after_verify only when
    # is_complete=True, but the reflect call sites use it mid-run, where the
...
async def _llm_reflect(
    ...
    try:
        ...
        analysis = await extractor.extract(
            response.content, ReflectionAnalysis, gateway=gateway, messages=messages
        )
        if analysis is None:
            return None
