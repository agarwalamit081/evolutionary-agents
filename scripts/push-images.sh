#!/usr/bin/env bash
# Push the locally-built images (self-evolving-agent / self-evolving-agent-runner /
# self-evolving-agent-toolbox) to the local registry defined in docker-compose.yml (the
...
#   scripts/push-images.sh self-evolving-agent self-evolving-agent-runner
set -euo pipefail
...
if [ "$#" -gt 0 ]; then
  ...
  IMAGES=(self-evolving-agent self-evolving-agent-runner self-evolving-agent-toolbox)
fi
