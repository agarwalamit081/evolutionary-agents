            ...
            CheckConfig(
                ...
                params={
                    ...
                    "code": (
                        ...
                        "            break\n"
                        "# The q01 goal is filename-agnostic about the generated raw\n"
                        "# event log (\"Generate a ... event JSONL ...\"); an agent may\n"
                        "# name it seed_events.jsonl / events.jsonl / etc. Resolve any\n"
                        "# *.jsonl under root whose records carry an event id, picking\n"
                        "# the one with the most event records (the raw seed dominates\n"
                        "# intermediates). False-positive-safe: the checked==0 guard\n"
                        "# below still fails the probe if the pick shares no event_ids\n"
                        "# with normalized.csv.\n"
                        "if not raw and norm and _RESULTS_ROOT:\n"
                        "    _jsonls = []\n"
                        "    for _d, _, _fs in os.walk(_RESULTS_ROOT):\n"
                        "        for _fn in _fs:\n"
                        "            if _fn.endswith('.jsonl'):\n"
                        "                _jsonls.append(os.path.join(_d, _fn))\n"
                        "    def _evcount(_p):\n"
                        "        _n = 0\n"
                        "        try:\n"
                        "            with open(_p) as _gf:\n"
                        ...
                        "                    except ValueError:\n"
                        "                        continue\n"
                        "                    if isinstance(_oo, dict) and (_oo.get('event_id') or _oo.get('id')):\n"
                        "                        _n += 1\n"
                        "        except OSError:\n"
                        "            pass\n"
                        "        return _n\n"
                        "    for _n, _p in sorted(((_evcount(_p), _p) for _p in _jsonls), reverse=True):\n"
                        "        if _n > 0:\n"
                        "            raw = _p\n"
                        "            break\n"
                        "if not norm:\n"
                        "    print('no normalized.csv'); sys.exit(1)\n"
                        "if not raw:\n"
                        "    print('no raw event JSONL found (cannot verify instant preservation)'); sys.exit(1)\n"
                        "\n"
                        "raw_inst = {}\n"
                        "with open(raw) as _f:\n"
                        "    for _line in _f:\n"
