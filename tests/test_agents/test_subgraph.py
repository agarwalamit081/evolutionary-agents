...
class TestRoutingFunctions:
    ...
    def test_route_after_reflect_sub_low_confidence_with_steps(self) -> None:
        """Low confidence WITH remaining steps retries execute."""
        from src.agents.subgraph import _route_after_reflect_sub

        state = _make_sub_state(
            pending_tool_gaps=[], confidence="low",
            plan_steps=["s1", "s2", "s3"], current_step_index=0,
            iteration_count=2, max_iterations=10,
        )
        result = _route_after_reflect_sub(state)
        assert result == "execute"

    def test_route_after_reflect_sub_low_confidence_exhausted_ends(self) -> None:
        """battery-04 q07 Run 5: low confidence with the plan EXHAUSTED must
        END, not loop back to execute. Retry execute with no remaining steps
        is a tight reflect↔execute loop that only ends at LangGraph's recursion
        limit (GraphRecursionError)."""
        from src.agents.subgraph import _route_after_reflect_sub

        # step_index == len(plan_steps) → no remaining steps
        state = _make_sub_state(
            pending_tool_gaps=[], confidence="low",
            plan_steps=["s1", "s2", "s3"], current_step_index=3,
            iteration_count=2, max_iterations=10,
        )
        result = _route_after_reflect_sub(state)
        assert result == "__end__"

    def test_route_after_reflect_sub_no_plan_low_confidence_ends(self) -> None:
        """Empty plan + low confidence (the delegate-spawned sub-agent that
        finished its work but cannot self-verify) must END, not retry execute."""
        from src.agents.subgraph import _route_after_reflect_sub
    ...
    def test_route_after_reflect_sub_max_iterations_ends(self) -> None:
        """At the iteration cap, reflect accepts the partial result (END) and
        never re-enters execute — even with low confidence and steps left."""
        from src.agents.subgraph import _route_after_reflect_sub

        state = _make_sub_state(
            pending_tool_gaps=[], confidence="low",
            plan_steps=["s1", "s2"], current_step_index=0,
            iteration_count=10, max_iterations=10,
        )
        result = _route_after_reflect_sub(state)
        assert result == "__end__"
