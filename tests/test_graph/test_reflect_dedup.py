...
class TestGroundShouldEvolve:
    ...
    regardless of step count. The 1A relaxation: a deliverable on disk fires
    even when the run logged transient errors (objective-success); errors still
    block for non-deliverable goals (step-count + confidence bar).
    """
    ...
    def test_errors_do_not_block_when_deliverable_on_disk(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Objective-success (1A relaxation): a recovered-error run that still
        produced its deliverable is exactly the success worth learning from —
        transient errors do NOT suppress evolution when the artifact is on disk.
        Errors still block for non-deliverable goals (see test_non_deliverable_*)."""
