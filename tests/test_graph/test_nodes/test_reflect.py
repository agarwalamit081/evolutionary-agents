...
class TestGroundShouldEvolve:
    ...
    A deliverable goal fires whenever the artifact is on disk — regardless of
    step count (N8 delegate case) AND regardless of transient errors (a
    recovered-error success is the richest learning signal — battery-03 saw
    evolution fire only on pristine runs because a blanket errors-gate sat
    ahead of the deliverable check). The errors-gate survives only for
    non-deliverable goals, where no artifact confirms success."""
    ...
    def test_true_with_errors_when_deliverable_on_disk(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Relaxation (battery-03): a run that recovered from transient errors
        yet still produced its deliverable on disk is objective success → evolve.
        Previously the blanket ``if errors: return False`` gate sat ahead of the
        deliverable check, so evolution fired only on pristine runs and the
        richest learning signal was discarded."""
        monkeypatch.setattr("src.graph.nodes.execute._deliverable_on_disk", lambda _p: True)
        assert _ground_should_evolve(
            False, "merge into results/q3_overview.md", self._steps(), ["boom"], Confidence.LOW
        ) is True

    def test_false_with_errors_when_deliverable_missing(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Deliverable goal + errors + artifact NOT on disk = genuine failure →
        still suppressed (relaxation only forgives errors when the objective was
        actually met)."""
        monkeypatch.setattr("src.graph.nodes.execute._deliverable_on_disk", lambda _p: False)
        assert _ground_should_evolve(
            False, "merge into results/q3_overview.md", self._steps(), ["boom"], Confidence.HIGH
        ) is False

    def test_false_with_errors_for_nondeliverable_goal(self) -> None:
        """Non-deliverable goal: no artifact confirms success, so a logged error
        means the run is ambiguous/failed → never crystallize it. (The errors-gate
        survives only on this path.)"""
        assert _ground_should_evolve(
            False, "Explain the system architecture.", self._steps(), ["boom"], Confidence.HIGH
        ) is False
